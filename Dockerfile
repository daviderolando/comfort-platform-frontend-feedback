...
# specify default command to run when we run the image (i.e. when we create a container)
...
# COPY --from=stage1 /usr/src/app/node_modules .

# BUILD
# docker build -t test-app -f Dockerfile .
# DOCKER_BUILDKIT=1 docker build -t test-app --file Dockerfile --output node_modules .
...
# Copy node_modules to host (manually)
docker cp 26a24ce8cd7f:/usr/src/app/node_modules ./node_modules
